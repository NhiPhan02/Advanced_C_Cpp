...
int stringCompare(const char *str1, const char *str2) {
   while (*str1 && (*str1 == *str2)) {      //str1 = Pham Duy   ; 'D' = 68 //str1 = Duy
                                            //str2 = Pham An    ; 'A' = 65 //str2 = Duyen
       str1++;                  
       str2++;
   }
...
int compareByID(const void *a, const void *b) {
   SinhVien *sv1 = (SinhVien *)a;       //sv1->id = 104
   SinhVien *sv2 = (SinhVien *)b;       //sv2->id = 100
   return sv1->id - sv2->id;
}

// Hàm sắp xếp chung
void sort(SinhVien array[], size_t size, int (*compareFunc)(const void *, const void *)) { //con trỏ hàm, hằng, void pointer
   int i, j;
   SinhVien temp;
   for (i = 0; i < size-1; i++)    
       for (j = i+1; j < size; j++)
...
int main() {
   ...
   printf("Sắp xếp theo điểm trung bình:\n");
   sort(danhSachSV, size, compareByDiemTrungBinh);
   display(danhSachSV, size);

   // Sắp xếp theo ID
   printf("Sắp xếp theo ID:\n");
